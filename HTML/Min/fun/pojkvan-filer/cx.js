...
if (!cX.library) {
    cX.library = {
        ...
        m_rnd: Math.round(Math.random() * 2147483647).toString(36) + Math.round(Math.random() * 2147483647).toString(36),
        m_widgetSpecs: {},
        m_customParameters: [],
        ...
        hasLocalStorage: function() {
            try { return typeof localStorage === 'object' && typeof localStorage.getItem === 'function'; } catch (e) { return false; }
        },

        hasSessionStorage: function() {
            try { return typeof sessionStorage === 'object' && typeof sessionStorage.getItem === 'function'; } catch (e) { return false; }
        },
        ...
        sendPageViewEvent: function(args) {
            ...
            try {
                if (location.href && document.referrer && (location.href != '') && (location.href == document.referrer)) {
                    isAutoRefresh = true;
                    if (location.hostname.indexOf('nsd.se') < 0) {
                        return;
                    }
                }
            } catch (e) { }
            ...
            var pageView = this.m_accountId + '_' + this.m_siteId + '_' + location.href;
            if (this.m_previousPageViewReport !== pageView) {
                this.m_previousPageViewReport = pageView;

                if (document.images || (location.hostname.indexOf('nsd.se') > -1)) {
                    var isNewUser = !(this.getUserId(false));
                    var sessionUserId = this.getSessionId();
                    var persistentUserId = this.getUserId();

                    var repSrc = this.eventReceiverBaseUrl + '?ver=1&typ=pgv&rnd=' + this.m_rnd;
                    if (location.hostname.indexOf('.cxense.com') > -1 || location.hostname.indexOf('.cxpublic.com') > -1 || Math.random() < 0.001) {
                        var repSrc = this.p1BaseUrl + '#ver=1&typ=pgv&rnd=' + this.m_rnd;
                    }
                    try { repSrc += '&acc=' + encodeURIComponent(this.m_accountId); } catch (e) { }
                    try { repSrc += '&sid=' + encodeURIComponent(this.m_siteId); } catch (e) { }
                    try { repSrc += '&loc=' + encodeURIComponent(allArgs.location || location.href); } catch (e) { }
        ...
        getSessionId: function(createIfMissing) {
            ...
            if (!sessionUserId && createIfMissing !== false) {
                sessionUserId = new Date().getTime() + new String(Math.round(Math.random() * 2147483647));
                try { this.setCookie('cX_S', sessionUserId, null, '/'); } catch (e) { }
        ...
        getUserId: function(createIfMissing) {
            var persistentUserId = this.getCookie('cX_P');
            if (!persistentUserId) {
                try { if (this.hasLocalStorage()) { persistentUserId = localStorage.getItem('_cX_P'); } } catch (e) { }
            }
            if (!persistentUserId && createIfMissing !== false) {
                persistentUserId = new Date().getTime() + new String(Math.round(Math.random() * 2147483647));
                try { this.setCookie('cX_P', persistentUserId, 365, '/'); } catch (e) { }
            ...
            try {
                if (typeof message === 'object' && typeof message.data === 'string') {
                    ...
                    if (typeof object.elementId !== 'undefined' && object.method !== 'updateAdSpace') {
                        this._logRemote({url: location.href, method: '' + object.method});
                    }
                }
            } catch (e) { }
        },
        ...
        invoke: function(func) {
            func.apply(window, Array.prototype.slice.call(arguments).slice(1));
        },
    ...
    function cx_pollParentMessage() {
        try {
            cX.library._onPollParentMessage();
        } catch (e) {}
